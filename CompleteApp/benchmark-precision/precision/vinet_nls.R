...
m<- nlsLM(E ~ ((aE*x)/(bE0+x) + 2 * (aB*x)/(bB0+x) * (aV*x)/(bV0+x) / ((aP*x)/(bP0+x) - 1)**2 *
          (2 - (5 + 3 * (aE*x)/(bE0+x) * ( (V / (aV*x)/(bV0+x))**(1 / 3) - 1) - 3 * (V / (aV*x)/(bV0+x))**(1 / 3) ) *
          exp(-3 * ((aP*x)/(bP0+x) - 1) * ( (V / (aV*x)/(bV0+x))**(1 / 3) - 1) / 2))), start = list(aE=aE,aB=aB,aP=aP,aV=aV, bE0=1, bB0=1, bP0=1, bV0=1))
