...
m <- nlsLM(E ~ (aE*x)/(bE0+x) + (aB*x)/(bB0+x) * (aV*x)/(bV0+x) * (-3 * log((V / (aV*x)/(bV0+x) )**(1 / 3)))**2 / 6 * (3 + (-3 * log((V / (aV*x)/(bV0+x) )**(1 / 3))) * ((aP*x)/(bP0+x) - 2)), start = list(aE=aE,aB=aB,aP=aP,aV=aV, bE0=1, bB0=1, bP0=1, bV0=1))
